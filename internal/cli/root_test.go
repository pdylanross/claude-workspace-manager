...
func TestVersionCommand(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cmd := cli.NewRootCmd(info, paths.New(paths.NewOSEnvironment()), stubUpdaterFactory(), stubToolFactory())
			cmd.SetOut(&out)
			cmd.SetErr(&out)
			cmd.SetArgs(tt.args)
...
func TestVersionCommandRejectsArgs(t *testing.T) {
	...
	cmd := cli.NewRootCmd(version.Info{}, paths.New(paths.NewOSEnvironment()), stubUpdaterFactory(), stubToolFactory())
	cmd.SetOut(&out)
	cmd.SetErr(&out)
